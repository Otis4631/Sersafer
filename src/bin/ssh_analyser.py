...
#     exit(0)
try:
	os.system('cat /var/log/auth.log.1 >> /var/log/auth.log')
except Exception as err:
	pass

try:
	log = openLog("/var/log/auth.log")
except Exception as err:
	log = openLog("/var/log/auth.log.1")
...
def KeyValueList2Html(kvlist, headerMap):
    ...
    if headerMap:
        ...
        debug(hkey)
        debug(hvalue)
    html+= "<th>"+"<td> <div color='red'>"+hkey+'</div></td>'+'<td>'+hvalue+'</td>'+ '</th>\n'
    for kv in kvlist:
...
reportFilename = 'ssh.html'

report = open("../lib/templates/"  + reportFilename, 'w')
if report:
...
""")
    ...
    report.write("<br/><p>------------ Tested user list *Failed* -------------</p>\n")
